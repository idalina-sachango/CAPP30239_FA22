...
d3.csv("../degrees.csv").then((data) => {
  ...
  let arr1 = d3.map(data, d => d.source), // find everywhere where it says source, and put it in a new array called arr1
    arr2 = d3.map(data, d => d.target); // same as above
  ...
  const arc = d3.arc() //shape generaotr. building a circle essentially
    .innerRadius(innerRadius)
    .outerRadius(outerRadius);

  // const ribbon = d3.ribbonArrow()
  const ribbon = d3.ribbon() // building the shape of the ribbons inside
  ...
  svg.append("g")
    .selectAll("g")
    .data(chords) // grabbing ribbon data
    .join("path")
    .attr("d", ribbon)
    ...
    .on("click", function(e, d) { // cahnges text on the page
