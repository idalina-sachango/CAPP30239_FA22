...
Promise.all([
  ...
  const radius = d3.scaleLinear() // how big the circle is.
    .domain([0, d3.max(Object.values(data), d => d.crimeDensity)])
    .range([0, 25]);

  const communities = topojson.feature(chicagoTopology, chicagoTopology.objects.chicago); // grab community boundaries
  const mesh = topojson.mesh(chicagoTopology, chicagoTopology.objects.chicago);
  const projection = d3.geoMercator() // adding a projection. how a country is displayed. albers is a type of projection.
    .fitSize([width, height], mesh); // mesh is what we defined above. 
